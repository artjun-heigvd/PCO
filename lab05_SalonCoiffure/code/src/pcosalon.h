...
protected:
    ...
    PcoMutex _mutex; //moniteur MESA

    // Les PcoConditionVariable qui vont être utilisées avec le mutex plus haut
    // afin de faire des moniteurs MESA
    PcoConditionVariable _zzzBarber;
    PcoConditionVariable _waitingForClient;
    PcoConditionVariable _cutting;
    PcoConditionVariable _waitingForCut;
    PcoConditionVariable _waitingToClose; // MESA pour que l'application attende avant de fermer

    bool _isSleeping{false}, _isOpen{true};
    // ticket et order sont utilisés pour garantir l'ordre de passage des clients
    unsigned _capacity, _nbInSaloon{0}, _ticket{0}, _order{0};
    std::vector<bool> _chairs; // Vecteur des chaises libres
};
