...
public:
    ...
    bool operator<(const Result& other) const {return id < other.id;}
    bool operator>(const Result& other) const {return id > other.id;}
...
protected:
    // bufferAnswers must be ordered by ids
    std::set<Result> bufferAnswers;
    // Map a computation type to a monitor condition
    std::map<ComputationType, Condition> waitFullComputation;
    std::map<ComputationType, Condition> waitEmptyComputation;
    Condition waitAnswer;
    std::map<ComputationType, std::list<Request>> bufferComputation;
...
private:
    ...
    // Ids generator for our computations
    int nextId = 0;
    // List of all the ids of the computations that we are waiting on
    std::list<int> waitingIds;
};
